...
export default function EditFIPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const res = await fetch(`/api/t/${tenant}/fi/${fiId}`, {
          cache: "no-store",
        });
        ...
        if (!res.ok) throw new Error(data?.error || "Failed to load FI");
        if (!isMounted) return;
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data?.error || "Failed to update FI");

      router.push(`/t/${tenant}/fi/${fiId}`);
    } catch (e: any) {
  ...
  const onRemovePhoto = async (photoId: string) => {
    ...
    try {
      const res = await fetch(`/api/t/${tenant}/fi/photo/${photoId}`, {
        method: "DELETE",
      });
